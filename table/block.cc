...
void BlockIter::Seek(const Slice& target) {
  ...
  if (data_ == nullptr) {  // Not init yet
    //std::cout << "Seek: data_ nullptr" << std::endl;
    return;
  }
  uint32_t index = 0;
  bool ok = false;
  if (prefix_index_) {
    //std::cout << "Seek:PrefixSeek" << std::endl;
    ok = PrefixSeek(target, &index);
  } else {
    //std::cout << "Seek:BinarySeek" << std::endl;
...
void BlockIter::SeekWithIndex(const Slice& target, const uint32_t begin_restart, const uint32_t end_restart,
                              const uint32_t equal_prefix_len, bool use_optimized_restart_range) {
  PERF_TIMER_GUARD(block_seek_nanos);
  if (data_ == nullptr) {  // Not init yet
    return;
  }
  ...
  if (prefix_index_) {
    ...
    uint64_t min_restart = 0, max_restart = num_restarts_ - 1;
    if(use_optimized_restart_range) {
      min_restart = std::max(size_t(0), size_t(begin_restart / 16));
      max_restart = std::min(size_t(end_restart / 16), size_t(num_restarts_ - 1));
    }
    ok = BinarySeek(target, min_restart, max_restart, &index, equal_prefix_len);
  }
  ...
  while (true) {
    ...
    if (!ParseNextKey() || Compare(key_.GetKey(), target, equal_prefix_len) >= 0) {
      return;
    }
  }
}

uint32_t BlockIter::BinarySearchInRestarts(uint32_t left, uint32_t right,
  ...
  while (left < right) {
    ...
    uint32_t region_offset = GetRestartPoint(mid);
      ...
      if (offset_index < region_offset) {
        right = mid - 1;
      } else if (offset_index > region_offset) {
        left = mid;
      } else {
        left = right = mid;
      }
...
bool BlockIter::BinarySeek(const Slice& target, uint32_t left, uint32_t right,
                           uint32_t* index, uint32_t equal_prefix_len) {
  assert(left <= right);
  //std::cout << "BinarySeek:target: " << target.ToString() << std::endl;
  //std::cout << "BinarySeek:left:right: " << left << "," << right << std::endl;
  ...
  while (left < right) {
    ...
    //std::cout << "BinarySeek:mid_key: " << mid_key.ToString() << std::endl;

    int cmp = Compare(mid_key, target, equal_prefix_len);
    //std::cout << "BinarySeek:cmp: " << cmp << std::endl;
