...
class BlockIter : public InternalIterator {
 public:
  ...
  void SeekWithIndex(const Slice& target, const uint32_t begin_restart, const uint32_t end_restart,
    const uint32_t equal_prefix_len, bool use_optimized_restart_range = false);
 ...
 private:
  ...
  inline int Compare(const Slice& a, const Slice& b, uint32_t prefix_size = 0) const {
    Slice a_trimmed(a.data() + prefix_size, a.size() - prefix_size);
    Slice b_trimmed(b.data() + prefix_size, b.size() - prefix_size);
    return comparator_->Compare(a_trimmed, b_trimmed);
  }
  ...
  uint32_t BinarySearchInRestarts(uint32_t left, uint32_t right,
  ...
  bool BinarySeek(const Slice& target, uint32_t left, uint32_t right,
                  uint32_t* index, uint32_t equal_prefix_len = 0);
