...
#include "learned_index/serializer.h"
...
struct BlockBasedTable::Rep {
  ...
  rss::RadixStringSpline learned_index_model;
  struct learned_index_block_data_item {
    const size_t offset;
    const size_t size;
    const size_t last_kv_index;
  };
  std::vector<learned_index_block_data_item> learned_index_block_data;
...
Status BlockBasedTable::Open(const ImmutableCFOptions& ioptions,
  ...
  Slice learned_index_content;
  uint64_t learned_index_handle_offset = rep->footer.learned_index_handle().offset();
  uint64_t learned_index_handle_size = rep->footer.learned_index_handle().size();
  std::unique_ptr<char> buf(new char[learned_index_handle_size]);
  s = rep->file->Read(learned_index_handle_offset, learned_index_handle_size, &learned_index_content, buf.get());
  //TODO (LI): pass character for performance efficiency
  rep->learned_index_model = rss::RSS_Serializer::FromBytes(learned_index_content.ToString());
 
 
  size_t block_num = 0;
  rep->learned_index_block_data.clear();
  if (s.ok()) {
    ...
    for (iiter->SeekToFirst(); iiter->Valid(); iiter->Next()) {
      ...
      rep->learned_index_block_data.push_back({
        handle.offset(), handle.size(), rep->learned_index_model.get_accumulate_block_kv_num(block_num)
      });
    }
  }
...
InternalIterator* BlockBasedTable::NewDataBlockIterator(
  ...
  Status s = MaybeLoadDataBlockToCache(rep, ro, handle, compression_dict, &block);
  ...
  if (!s.ok()) {
    Log(InfoLogLevel::ERROR_LEVEL, rep->ioptions.info_log,
        "NewDataBlockIterator return %s", s.ToString().c_str());
  }
  return iter;
}
...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  return GetUsingLearnedIndex(read_options, key, get_context, skip_filters);
}

Status BlockBasedTable::GetUsingLearnedIndex(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context, bool skip_filters) {
  Status s;
  CachableEntry<FilterBlockReader> filter_entry;
  if (!skip_filters) {
  ...
  if (!FullFilterKeyMayMatch(read_options, filter, key)) {
    ...
    auto model_output = rep_->learned_index_model.find(ExtractUserKey(key));
    GetKeyFromBlock(get_context, model_output, filter,
                    key, read_options, pinned_iters_mgr, pin_blocks, s);
  }
...
void BlockBasedTable::GetKeyFromBlock(rocksdb::GetContext* get_context,
    ...
    const auto rss_node = model_output.rss_node;
    int block_num = rss_node->GetBlockNum(key, model_output.spline);
    const size_t block_offset = rep_->learned_index_block_data[block_num].offset;
    const size_t block_size = rep_->learned_index_block_data[block_num].size;
    // std::cout <<"GetKeyFromBlock:block_num: " << block_num << ", block size:" << block_size << std::endl;
    BlockHandle handle(block_offset, block_size);

    const size_t block_idx_start = block_num <= 0 ? 0 : rep_->learned_index_block_data[block_num - 1].last_kv_index;
    const size_t block_idx_end = rep_->learned_index_block_data[block_num].last_kv_index;
    const auto search_bound = rss_node->GetBlockSearchBound(key, model_output.spline, block_idx_start, block_idx_end);
    //std::cout <<"GetKeyFromBlock:search_bound: " << search_bound.begin << "," << search_bound.end << std::endl;
    const size_t prefix_size = rss_node->GetPrefixLen();
    //std::cout <<"GetKeyFromBlock:prefix_size: " << prefix_size << std::endl;

    bool not_exist_in_filter =
        filter != nullptr && filter->IsBlockBased() == true &&
        !filter->KeyMayMatch(ExtractUserKey(key), handle.offset());
    //std::cout << "GetKeyFromBlock:not_exist_in_filter:" << std::endl;
  ...
  do {
    if (not_exist_in_filter) {
      ...
      if (read_options.read_tier == kBlockCacheTier &&
        ...
        //std::cout << "GetKeyFromBlock:biter.status().IsIncomplete" << std::endl;
      ...
      if (!biter.status().ok()) {
        //std::cout << "GetKeyFromBlock:!biter.status().ok" << std::endl;
        s = biter.status();
        Log(InfoLogLevel::ERROR_LEVEL, rep_->ioptions.info_log,
            "init block err,%s, handle(%llu,%llu)", s.ToString().c_str(),
      ...
      for (biter.Seek(key); biter.Valid(); biter.Next()) {
        //std::cout << "GetKeyFromBlock:biter.Seek" << std::endl;
        ParsedInternalKey parsed_key;
        if (!ParseInternalKey(biter.key(), &parsed_key)) {
          s = Status::Corruption("Parse internal key fail");
        }

        if (!get_context->SaveValue(parsed_key, biter.value(), pin_blocks)) {
          //std::cout << "GetKeyFromBlock:!get_context->SaveValue" << std::endl; 
          break;
        }
      }
      s = biter.status();
      if (!s.ok()) {
        //std::cout << "GetKeyFromBlock:!s.ok()" << std::endl; 
        Log(InfoLogLevel::WARN_LEVEL, rep_->ioptions.info_log,
            "go through block,%s, handle(%llu,%llu)", s.ToString().c_str(),
            handle.offset(), handle.size());
        break;
      }
